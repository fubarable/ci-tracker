...
function toggle(source: Source) {
    router.patch(`/settings/input-sources/${source.id}/toggle`, {}, { preserveScroll: true });
}

function remove(source: Source) {
    if (confirm(`Delete "${source.name}"? This cannot be undone.`)) {
        router.delete(`/settings/input-sources/${source.id}`, { preserveScroll: true });
    }
}
</script>
    ...
    <div class="p-6 space-y-6 max-w-2xl">
        ...
        <Card>
            ...
            <CardContent class="flex gap-2">
                <Input v-model="newName" placeholder="e.g. My Podcast" @keyup.enter="addSource" />
        ...
        <Card>
            ...
            <CardContent>
                <ul class="divide-y">
                    <li v-for="s in props.sources" :key="s.id" class="py-2 flex items-center justify-between text-sm">
                        <div class="flex items-center gap-2">
                            <span :class="{ 'text-muted-foreground line-through': !s.is_active }">{{ s.name }}</span>
                            <Badge v-if="s.is_system" variant="secondary">system</Badge>
                        </div>
                        <div v-if="!s.is_system" class="flex gap-2">
                            <Button variant="ghost" size="sm" @click="toggle(s)">
                                {{ s.is_active ? 'Deactivate' : 'Activate' }}
                            </Button>
                            <Button variant="link" size="sm" class="text-destructive" @click="remove(s)">
                                Delete
                            </Button>
                        </div>
                    </li>
